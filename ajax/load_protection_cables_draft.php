...
try {
    ...
    // Load protection cables draft
    $stmt = $pdo->prepare("
        SELECT draft_data FROM drafts
        WHERE report_id = ? AND draft_type = 'protection_cables'
    ...
    if ($result) {
        echo json_encode([
            'success' => true,
            'protection_cables_data' => $result['draft_data']
        ]);
    } else {
        echo json_encode([
            'success' => false,
            'message' => 'No protection cables data found'
        ]);
    }
} catch (Exception $e) {
